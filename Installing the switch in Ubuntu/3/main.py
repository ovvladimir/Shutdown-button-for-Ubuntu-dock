...
com = '''\
python3 -c "from tkinter import Tk, PhotoImage, Frame, Label, Button; from os import system, path; \
t = Tk(); t.overrideredirect(True); \
img = PhotoImage(file=path.join(path.expanduser('~'), '.local/share/applications/icon.png')); \
txt = 'Выключение или перезагрузка П-166М и компьютера'; clr = '#6495ED'; w = '#fff'; \
l1 = ('Перезагрузка', 'Отмена', ' Выкл '); l2 = (lambda: f('reboot'), t.destroy, lambda: f('poweroff')); \
f = lambda a: system('killall -s SIGINT P166MClient; killall -s SIGINT P166MPlan; sleep 1; killall python3 && {}'.format(a)); \
