...
com = '''\
python3 -c "from tkinter import Tk, PhotoImage, Frame, Label, Button; from os import system; \
t = Tk(); t.overrideredirect(True); \
img = PhotoImage(file='/usr/share/icons/Yaru/48x48@2x//actions/system-shutdown.png'); \
txt = 'Выключение или перезагрузка компьютера'; clr = '#6495ED'; w = '#fff'; \
l1 = ('Перезагрузка', 'Отмена', ' Выкл '); l2 = (lambda: f('reboot'), t.destroy, lambda: f('poweroff')); \
f = lambda a: system(f'killall python3 && {a}'); fr = Frame(t, bg=w, bd=2); fr.pack(); \
